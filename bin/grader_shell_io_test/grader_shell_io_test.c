...
int main(void) {
    ...
    {
        sh_c("echo redirect_test > /tmp_shell_out");
        int fd = open("/tmp_shell_out", O_RDONLY);
        ...
        unlink("/tmp_shell_out");
    }

    /* 4. Append redirection: >> */
    {
        sh_c("echo line1 > /tmp_shell_app");
        sh_c("echo line2 >> /tmp_shell_app");
        int fd = open("/tmp_shell_app", O_RDONLY);
        ...
        unlink("/tmp_shell_app");
    }

    /* 5. Input redirection: < file */
    {
        /* Create a file, then cat < file */
        int fd = open("/tmp_shell_in", O_CREAT | O_WRONLY);
        if (fd >= 0) {
            write(fd, "input_data\n", 11);
            close(fd);
        }
        int rc = sh_c("cat < /tmp_shell_in > /tmp_shell_in_out");
        if (rc == 0) {
            fd = open("/tmp_shell_in_out", O_RDONLY);
            if (fd >= 0) {
                char buf[64] = {0};
                read(fd, buf, sizeof(buf) - 1);
                close(fd);
    ...
    {
        int rc = sh_c("echo pipe_test | cat > /tmp_shell_pipe");
        if (rc == 0) {
            int fd = open("/tmp_shell_pipe", O_RDONLY);
            if (fd >= 0) {
                char buf[64] = {0};
                read(fd, buf, sizeof(buf) - 1);
                close(fd);
