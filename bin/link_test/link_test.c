...
int main(void) {
    ...
    try_unlink("/mnt/lt_b");
    try_unlink("/mnt/lt_c");
    try_unlink("/mnt/lt_dir2");
    try_unlink("/mnt/lt_dir");

    /* ---- 1. Create lt_a with content "hello". ---- */
    int fd = open("/mnt/lt_a", O_WRONLY | O_CREAT);
    chk(fd >= 0, "create /mnt/lt_a");
    if (fd >= 0) { write(fd, "hello", 5); close(fd); }

    struct stat st;
    chk(stat("/mnt/lt_a", &st) == 0 && st.st_nlink == 1,
        "lt_a nlink == 1 before link");

    /* ---- 2. link lt_a → lt_b succeeds. ---- */
    int rc = link("/mnt/lt_a", "/mnt/lt_b");
    chk(rc == 0, "link /mnt/lt_a -> /mnt/lt_b returns 0");

    /* ---- 3. nlink == 2 on both names. ---- */
    chk(stat("/mnt/lt_a", &st) == 0 && st.st_nlink == 2,
        "lt_a nlink == 2 after link");
    chk(stat("/mnt/lt_b", &st) == 0 && st.st_nlink == 2,
        "lt_b nlink == 2 after link");

    /* ---- 4. Same inode (st_ino matches). ---- */
    struct stat st2;
    if (stat("/mnt/lt_a", &st) == 0 && stat("/mnt/lt_b", &st2) == 0)
        chk(st.st_ino == st2.st_ino, "lt_a and lt_b share inode");

    /* ---- 5. Read from lt_b returns "hello". ---- */
    fd = open("/mnt/lt_b", O_RDONLY);
    chk(fd >= 0, "open /mnt/lt_b");
    if (fd >= 0) {
        char buf[16] = {0};
    ...
    fd = open("/mnt/lt_a", O_WRONLY | O_TRUNC);
    ...
    chk(unlink("/mnt/lt_a") == 0, "unlink lt_a");
    chk(stat("/mnt/lt_a", &st) < 0, "lt_a gone after unlink");
    chk(stat("/mnt/lt_b", &st) == 0 && st.st_nlink == 1,
