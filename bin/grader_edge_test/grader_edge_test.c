...
int main(void) {
    ...
    {
        int fd = open("/tmp_edge_write", O_CREAT | O_WRONLY);
        ...
        if (fd >= 0) {
            ...
            close(fd);
            unlink("/tmp_edge_write");
        }
    }

    /* 2. read returns the correct byte count */
    {
        int fd = open("/tmp_edge_read", O_CREAT | O_WRONLY);
        if (fd >= 0) {
            write(fd, "hello", 5);
            close(fd);
            fd = open("/tmp_edge_read", O_RDONLY);
            if (fd >= 0) {
                char buf[64];
            ...
            unlink("/tmp_edge_read");
        }
    }

    /* 3. stat st_size is correct */
    {
        int fd = open("/tmp_edge_stat", O_CREAT | O_WRONLY);
        if (fd >= 0) {
            ...
            close(fd);
            unlink("/tmp_edge_stat");
        }
    }

    /* 4. Multiple writes accumulate — not overwrite */
    {
        int fd = open("/tmp_edge_accum", O_CREAT | O_WRONLY);
        if (fd >= 0) {
            write(fd, "aaa", 3);
            write(fd, "bbb", 3);
            write(fd, "ccc", 3);
            ...
            fd = open("/tmp_edge_accum", O_RDONLY);
            if (fd >= 0) {
                char buf[16] = {0};
            ...
            unlink("/tmp_edge_accum");
        }
    }

    /* 5. O_TRUNC clears existing file */
    {
        int fd = open("/tmp_edge_trunc", O_CREAT | O_WRONLY);
        if (fd >= 0) {
            write(fd, "old data here", 13);
            close(fd);

            fd = open("/tmp_edge_trunc", O_WRONLY | O_TRUNC);
            if (fd >= 0) {
                write(fd, "new", 3);
                close(fd);

                fd = open("/tmp_edge_trunc", O_RDONLY);
                if (fd >= 0) {
                    char buf[32] = {0};
            ...
            unlink("/tmp_edge_trunc");
        }
    }

    /* 6. O_APPEND writes at end */
    {
        int fd = open("/tmp_edge_append", O_CREAT | O_WRONLY);
        if (fd >= 0) {
            write(fd, "first", 5);
            close(fd);

            fd = open("/tmp_edge_append", O_WRONLY | O_APPEND);
            if (fd >= 0) {
                write(fd, "second", 6);
                close(fd);

                fd = open("/tmp_edge_append", O_RDONLY);
                if (fd >= 0) {
                    char buf[32] = {0};
                    read(fd, buf, 32);
    ...
    {
        int fd = open("/tmp_edge_forkfd", O_CREAT | O_RDWR);
        if (fd >= 0) {
            write(fd, "shared", 6);
            int pid = fork();
            if (pid == 0) {
