...
int main(void) {
    ...
    int fd = open("/tmp_grader_test", O_CREAT | O_WRONLY);
    ...
    if (fd >= 0) {
        ...
        fd = open("/tmp_grader_test", O_RDONLY);
        ...
        unlink("/tmp_grader_test");
    }

    /* 2. lseek */
    fd = open("/tmp_grader_seek", O_CREAT | O_RDWR);
    if (fd >= 0) {
        write(fd, "abcdefgh", 8);
        ...
        close(fd);
        unlink("/tmp_grader_seek");
    }

    /* 3. fstat */
    fd = open("/tmp_grader_stat", O_CREAT | O_WRONLY);
    if (fd >= 0) {
        write(fd, "test", 4);
        struct stat st;
        int rc = fstat(fd, &st);
    ...
    /* 5. dup */
    fd = open("/tmp_grader_dup", O_CREAT | O_RDWR);
    if (fd >= 0) {
        int fd2 = dup(fd);
        ...
        close(fd2);
        close(fd);
        unlink("/tmp_grader_dup");
    }

    /* 6. dup2 */
    fd = open("/tmp_grader_dup2", O_CREAT | O_RDWR);
    if (fd >= 0) {
        int rc = dup2(fd, 10);
        ...
        close(10);
        close(fd);
        unlink("/tmp_grader_dup2");
    }

    /* 7. unlink makes file disappear */
    fd = open("/tmp_grader_unlink", O_CREAT | O_WRONLY);
    if (fd >= 0) {
        close(fd);
        int rc = unlink("/tmp_grader_unlink");
        check(rc == 0, "unlink succeeds");
        errno = 0;
        fd = open("/tmp_grader_unlink", O_RDONLY);
    ...
    {
        FILE *fp = fopen("/tmp_grader_stdio", "w");
        check(fp != 0, "fopen(w) succeeds");
        if (fp) {
            fprintf(fp, "line1\n");
            fclose(fp);
            fp = fopen("/tmp_grader_stdio", "r");
